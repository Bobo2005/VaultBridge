...
async function main() {
  ...
  // We'll use a mock ERC20 token for testing
  // In production, this would be a real token like cUSDC
  const MockERC20 = await hre.ethers.getContractFactory("src/creditcoin/MockERC20.sol:MockERC20");
  ...
  const VaultLending = await hre.ethers.getContractFactory("src/creditcoin/VaultLending.sol:VaultLending");
  ...
  console.log(`Deployer MTK balance: ${hre.ethers.utils.formatEther(balance)}`);
