...
export const TopBar: React.FC<TopBarProps> = ({
  ...
  return (
    <>
      <header
        className={`space-y-4 pb-5 sm:pb-6 border-b border-border mb-6 sm:mb-8 ${className}`}
      >
        {/* Tier 1: Title, Subtitle, and Primary Actions */}
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex-1 min-w-0">
            <h1 className="text-2xl sm:text-3xl font-bold text-ink tracking-tight whitespace-normal break-normal leading-tight">
              {title}
            </h1>
            {subtitle && (
          ...
          {/* Primary Action Button & Main Wallet Pill */}
          <div className="flex items-center flex-wrap gap-2.5 shrink-0">
            {actionButton}

            {isConnected ? (
              <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-primary-tint border border-primary/20 rounded-btn text-xs font-semibold text-primary shadow-xs">
        ...
        {/* Tier 2: Interactive Toolbar (Faucet, Judge Speedrun, Wallet Utilities, Network Switcher, Filters) */}
        <div className="flex items-center justify-between flex-wrap gap-2.5 pt-2 border-t border-border/50 text-xs">
          {/* Left Toolbar Tools */}
          <div className="flex items-center flex-wrap gap-2">
            ...
            <button
              ...
              className="p-1.5 bg-surface hover:bg-bg border border-border rounded-btn text-ink-secondary hover:text-primary transition-all shadow-xs"
              title="Address Book"
            >
          ...
          {/* Right Toolbar Tools: Network & Filter */}
          <div className="flex items-center flex-wrap gap-2">
            ...
            {showFilter && (
              <div className="relative">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-surface border border-border rounded-btn text-xs font-semibold text-ink-secondary hover:text-ink hover:border-slate-300 transition-all shadow-xs cursor-pointer"
                >
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-surface hover:bg-bg border border-border rounded-btn text-xs font-medium text-ink-secondary shadow-xs transition-colors cursor-pointer"
