...
def build_docx_from_markdown():
    md_path = r"c:\Users\USER\Downloads\VaultBridge\MASTER_PROJECT_DOC.md"
    docx_path = r"c:\Users\USER\Downloads\VaultBridge\MASTER_PROJECT_DOC.docx"
    ...
    while i < len(lines):
        ...
        if line.startswith("```"):
            if in_code_block:
                # End of code block - render in a single-cell table with background
                in_code_block = False
                code_text = "\n".join(code_lines)
        ...
        if line.startswith("- ") or line.startswith("* ") or re.match(r"^\d+\.\s", line):
            ...
            for part in parts:
                if part.startswith("**") and part.endswith("**"):
                    ...
                    r.font.size = Pt(9)
                    r.font.color.rgb = RGBColor(185, 28, 28) # Red-600 code
                else:
